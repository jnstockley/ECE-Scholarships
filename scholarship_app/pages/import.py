...
def display_file_upload():
    ...
    st.write(
        "Add files to be imported. Multiple files can be selected and the application will attempt to merge them."
    )
    form = st.form(key="import_data_form")
    files = form.file_uploader(
        "Data Sources:", accept_multiple_files=True, type=["xlsx", "csv", "tsv"]
    )
    form.markdown(
...
def display_alignment_column_form():
    ...
    if alignment_form_submit:
        ...
        alignment_info = AlignmentManager(
            drop_missing_checkbox, final_column_name_input, alignment_inputs
        )
...
def display_duplicate_column_form():
    ...
    duplicate_handler_form.write(
        f"For the unique alignment column value {duplicate_details.alignment_row_value}, please select which data to keep:"
    )
    ...
    with col2:
        value_selection = st.radio(
            f"Select the final value for column '{duplicate_details.duplicate_column_name}'",
...
def display_merge_form(similar_details: MergeSimilarDetails):
    ...
    st.write(
        "We have detected the following columns to be similar in name. Would you like to merge them?"
    )
    st.write(f"_{SESSION.similar.remaining_count()} remaining..._")

    percent_different = (
        similar_details.get_different_row_count() / len(SESSION.aligned_df.index)
    ) * 100
    merge_form = st.form(key="merge_data_form")
    merge_form.header("Useful Metrics:")
    merge_form.write(
        f"Of the {len(SESSION.aligned_df.index)} total rows, {similar_details.get_different_row_count()} have different"
    ...
    with merge_form.expander("Help me!"):
        st.write(
            "You can make changes to the FINAL COLUMN values by double clicking on a cell and entering the new preferred value! "
            + "Any values you set in this column will be the values used if you select to merge all similar columns."
        )

    all_columns = (
        similar_details.aligned_df.copy(deep=True)
        .drop(columns=[similar_details.alignment_col])
        .columns
    )

    # Select columns input
    select_columns_container = merge_form.container()
    selected_columns = select_columns_container.multiselect(
        "Select Columns to Merge", all_columns, similar_details.selected_columns
    )
    ...
    edited_final_col = merge_form.experimental_data_editor(
        similar_details.get_comparison_table()
    )

    render_script_expander(merge_form, similar_details)

    final_column_name = merge_form.text_input(
        "Final column name:", value=similar_details.final_column_name
    )
    ...
    if skip_button:
        ...
        if len(selected_columns) <= 1:
            select_columns_container.write(
                "Please select atleast two columns to merge. If you don't want to merge any columns, you can press skip at the bottom of the form."
            )
            return
