...
class DuplicateColumnData:
    ...
    def get_values(self) -> set[any]:
        ...
        for alignment_data in self.affected_alignment_data:
            ...
            values.append(
                data.loc[
                    data[alignment_data.column] == self.alignment_row_value,
    ...
    def get_comparison_df(self) -> pd.DataFrame:
        ...
        for selected_alignment in self.affected_alignment_data:
            ...
            cell_value = ref_df.loc[
                ref_df[selected_alignment.column] == self.alignment_row_value,
                self.duplicate_column_name,
            ].tolist()[0]

            data.loc[
                self.duplicate_column_name, selected_alignment.sheet.file_name
            ] = cell_value
...
class AlignmentManager:
    ...
    def __init__(
        ...
        self.info = AlignmentInfo(
            drop_missing, final_column_name, selected_alignment_columns
        )

        self.combine_columns = [
            setting.get_alignment_col() for setting in selected_alignment_columns
        ]
        self.final_alignment_column = merge.combine_columns(
            self.combine_columns, self.info.drop_missing
        )

        self._duplicate_df_columns = self._find_duplicate_columns()
        self._mismatched_duplicate_column_rows: list[
            DuplicateColumnData
        ] = self._find_duplicate_columns_with_differing_data()
    ...
    def select_duplicate_value(
        self, duplicate_info: DuplicateColumnData, selected_value: any
    ):
        """
        ...
        for alignment_data in duplicate_info.affected_alignment_data:
            data = alignment_data.sheet.get_df()
            data.loc[
                data[alignment_data.column] == duplicate_info.alignment_row_value,
                duplicate_info.duplicate_column_name,
            ] = selected_value
    ...
    def get_aligned_df(self) -> pd.DataFrame:
        ...
        """
        alignment_columns = [
            selected_alignment.column
            for selected_alignment in self.info.selected_alignment_columns
        ]
        sheets = [
            selected_alignment.sheet
            for selected_alignment in self.info.selected_alignment_columns
        ]
        return merge.merge_with_alignment_columns(
            self.info.final_column_name,
            alignment_columns,
            self.final_alignment_column,
            sheets,
        )
    ...
    def _find_duplicate_columns(self) -> set[str]:
        ...
        return {
            column
            for column in set.intersection(*columns)
            if column not in alignment_columns
        }
        ...
        for _index, value in self.final_alignment_column.items():
            ...
            for duplicate_column in self._duplicate_df_columns:
                ...
                for selected_alignment in self.info.selected_alignment_columns:
                    data = selected_alignment.sheet.get_df()
                    if (
                        duplicate_column in data.columns.tolist()
                        and value in data[selected_alignment.column].tolist()
                    ):
                ...
                for data in affected_alignment_data:
                    cur_val = (
                        data.sheet.get_df()
                        .loc[
                            data.sheet.get_df()[data.column] == value, duplicate_column
                        ]
                        .tolist()[0]
                    )
                    ...
                    if prev_val is None:
                        ...
                        mismatches.append(
                            DuplicateColumnData(
                                value, duplicate_column, affected_alignment_data
                            )
                        )
                        break
