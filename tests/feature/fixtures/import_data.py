...
def import_similar_column_page(page: Page) -> Page:
    ...
    expect(page.get_by_text("Duplicate Column(s) Found")).to_have_text(
        "Duplicate Column(s) Found", timeout=10000
    )
...
def handle_similar_columns(page: Page, press_option: str = "skip"):
    ...
    remaining_label = page.get_by_role("paragraph").filter(
        has_text=re.compile(".+ remaining...")
    )
...
def skip_all_similar_import_complete_page(import_similar_column_page: Page) -> Page:
    ...
    expect(import_similar_column_page.get_by_text("import completed!")).to_have_text(
        "import completed!"
    )
