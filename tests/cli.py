...
def run(test: str = 'all'):
    ...
    if test == 'all':
        ...
        streamlit_process = subprocess.Popen(streamlit_cmd, stdout=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)

        time.sleep(4)
        subprocess.run(f"poetry run {CMD['PLAYWRIGHT']}", stdout=subprocess.PIPE, stdout=subprocess.PIPE, check=False, shell=True)
        ...
        subprocess.run(poetry_pyunit_cmd, stdout=subprocess.PIPE, stdout=subprocess.PIPE, check=False, shell=True)
    else:
