...
# Global variables; majors contains all the majors, group options is all the column names that can be selected for a
# group and group help is the help message when hovering over the ? on a group field.
...
def display_create():
    """
    This function displays all the associated view/actions for creating a scholarship
    ...
    # NOTE: text_input is used instead of number_input because I feel like the stepping functionality is unnecessary
    # and could throw them off when they see it, or they could accidentally hit it. This can be changed later if it is
    # found useful.
    ...
    # NOTE: I chose up to 400 for the RAI, but the highest value in the sample data is 363, I am not sure if RAI has
    # a hard max score.
    ...
    sat_comb = st.select_slider('Select the minimum SAT Combined requirement', value=(sat_math + sat_reading),
                                options=(x * 10 for x in range(0, 161)))
    # NOTE: 5.0 was chosen as the max for the GPA requirement, but there exists a couple students above 5.0. This
    # could be increased if necessarily, but I doubt any scholarships require above 5.0, or even above 4.0,
    # as that would greatly limit students unfairly.
    ...
    if st.button('Create Scholarship', key='Create Scholarship'):
        ...
        if name == "" or total == "" or value == "":
            ...
            scholarship = pd.Series(
                data=[name, total, value, rai, admit_score, major, act_math, act_english, act_comp, sat_math,
                      sat_reading,
...
def display_edit():
    """
    This function displays all the associated view/actions for editing a scholarship
    ...
    if button('Edit This Scholarship', key='Edit This Scholarship'):
        ...
        if edit_sch is None:
            ...
            total = st.text_input("New total amount of Scholarships", value=values['Total Amount'], max_chars=8,
                                  placeholder="Enter Numerical Amount")
            value = st.text_input('New value of each individual Scholarship', value=values['Value'], max_chars=8,
                                  placeholder="Enter Numerical Amount")
            rai = st.select_slider('Select the minimum RAI requirement', value=values['RAI'],
                                   options=(x * 5 for x in range(0, 81)))
            admit_score = st.select_slider('Select the minimum Admit Score requirement', value=values['Admit Score'],
                                           options=range(0, 37))
            major = st.selectbox("New majors the scholarship applies to", index=MAJORS.index(values['Major']),
                                 options=MAJORS)
            act_math = st.select_slider('Select the minimum ACT Math requirement', value=values['ACT Math'],
                                        options=range(0, 37))
            act_english = st.select_slider('Select the minimum ACT English requirement', value=values['ACT English'],
                                           options=range(0, 37))
            act_comp = st.select_slider('New minimum ACT Composite requirement', value=values['ACT Composite'],
                                        options=range(0, 37))
            sat_math = st.select_slider('New minimum SAT Math requirement', value=values['SAT Math'],
                                        options=(x * 10 for x in range(0, 81)))
            sat_reading = st.select_slider('New minimum SAT Reading requirement', value=values['SAT Reading'],
                                           options=(x * 10 for x in range(0, 81)))
            sat_comb = st.select_slider('Select the minimum SAT Combined requirement', value=values['SAT Combined'],
                                        options=(x * 10 for x in range(0, 161)))
            gpa = st.select_slider('New minimum GPA requirement', value=values['GPA'],
                                   options=(x / 20 for x in range(0, 101)))
            hs_percentile = st.select_slider('Select the minimum highschool percentile', value=values['HS Percentile'],
                                             options=(x for x in range(0, 101)))
            group1 = st.multiselect("Choose Group One", options=GROUP_OPTIONS,
                                    default=groups_string_to_list(values['Group One']), help=GROUP_HELP)
            group2 = st.multiselect("Choose Group Two", options=GROUP_OPTIONS,
                                    default=groups_string_to_list(values['Group Two']), help=GROUP_HELP)
            group3 = st.multiselect("Choose Group Three", options=GROUP_OPTIONS,
                                    default=groups_string_to_list(values['Group Three']), help=GROUP_HELP)
            if st.button('Finalize Changes', key='Finalize Changes'):
                edit_row(scholarships, index,
                         [('Total Amount', total), ('Value', value), ('RAI', rai), ('Admit Score', admit_score),
                          ('Major', major),
                          ('ACT Math', act_math), ('ACT English', act_english), ('ACT Composite', act_comp),
                          ('SAT Math', sat_math),
                          ('SAT Reading', sat_reading), ('SAT Combined', sat_comb), ('GPA', gpa),
                          ('HS Percentile', hs_percentile),
                          ('Group One', str(group1)), ('Group Two', str(group2)), ('Group Three', str(group3))])
                # We changed the values in our scholarships dataframe, but have not updated the actual file,
